if getgenv().Ocalism_dandy then warn("Ocalism: Already Executed") return end
  ...
  Button.MouseButton1Click:Connect(function()
    if not Active then
      ...
      if lp.Character and huma and workspace.CurrentCamera then
        --game:GetService("Players").LocalPlayer.TempPlayerStatsModule.Ragdoll
        huma.AutoRotate = false
        Button.Image = States.On
        Cursor.Visible = true
        
        lp.Character.HumanoidRootPart.CFrame = CFrame.new(lp.Character.HumanoidRootPart.Position,Vector3.new(workspace.CurrentCamera.CFrame.LookVector.X * MaxLength,lp.Character.HumanoidRootPart.Position.Y,workspace.CurrentCamera.CFrame.LookVector.Z * MaxLength))
    ...
    save()  
        ...
        if getgenv().config.skill_check then  
            spawn(function()  
                ...
                if screen then screen = screen:FindFirstChild("Menu") end  
                if not screen then return end  

                local check_frame = screen:FindFirstChild("SkillCheckFrame")  
                if not check_frame then return end  

                frameConnect = check_frame:GetPropertyChangedSignal("Visible"):Connect(function()  
                        ...
                        if generatorFolder then
                            for _, generator in ipairs(generatorFolder:GetChildren()) do
                                if generator.Name == "Generator" then
                                    local stat = generator:FindFirstChild("Stats") and generator.Stats:FindFirstChild("Completed")
  ...
  plr:AddToggle("speed",
    {
      ...
      spawn(function()
        while getgenv().config.speed do task.wait()
  ...
  plr:AddToggle("jump",
    {
      ...
      spawn(function()
        if getgenv().config.jump then
          while getgenv().config.jump do task.wait()
